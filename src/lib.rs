...
#[derive(Clone, Debug, serde::Serialize, serde::Deserialize, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct IpRangesV6 {
    firsts: Vec<u128>,
    lasts: Vec<u128>,
}

impl IpRangesV6 {
    pub fn new_from_cidr(cidr: Vec<cidr_utils::cidr::Ipv6Cidr>) -> Self {
        let intervals = cidr.iter().map(|cidr| rust_lapper::Interval {
            start: cidr.first(),
            stop: cidr.last(),
            val: 0,
    ...
    pub fn new(mut firsts: Vec<u128>, mut lasts: Vec<u128>) -> Result<Self> {
        firsts.sort();
        lasts.sort();
        firsts.dedup();
        lasts.dedup();
    ...
    pub fn contains_ip(&self, ip: std::net::Ipv6Addr) -> bool {
